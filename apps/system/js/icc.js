...
var icc = {
  ...
  hideViews: function icc_hideViews() {
    if (this.screenLockedBeforeModal) {
  ...
  input: function(message, timeout, options, callback) {
    ...
    this.screenLockedBeforeModal =
      document.getElementById('screen').classList.contains('locked');
    if (this.screenLockedBeforeModal) {
      document.getElementById('screen').classList.remove('locked');
    }
